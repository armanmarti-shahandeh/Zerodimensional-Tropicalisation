...
function local_field_expansion(f::AbstractAlgebra.Generic.Poly{<:AbstractAlgebra.Generic.PuiseuxSeriesFieldElem}, w::QQFieldElem, precision::QQFieldElem)
    ...
    if w >= precision
        ...
        #println("evaluate: ",evaluate(f, x*t^w))
        ...
        for c in roots(h) 
            ...
            if isempty(nextExponents) #This accounts for the case where we have computed a (finite) root in its entirety, in which case we will not have a valid next exponent to consider.
                push!(newRoots, c*t^w)
            else
                if iszero(first(coefficients(g))) #This accounts for the specific case where we have two roots, at least one of which is finite, such that the roots agree entirely in finite part, and this ensures that the shorter, finite root will be counted: e.g. f = (x-(1+t))(x-(1+t+t^3))
                    push!(newRoots, c*t^w)
                end
                for tailTerm in local_field_expansion(g, minimum(nextExponents), precision)
...
R, x = K["x"]
r1 = 1+t+t^2+t^3
r2 = 1+t^2+t^3 #For some reason, when I add a O(t^4) term to this, the function fails to return the imprecision factor, presumably as it is terminating under the presumption of no valid next exponents... Why does the introduction of this remove the next exponent of 3? I think this would be resolved by a prospective polynomial imprecision checking function...
f = (x-r1)*(x-r2)
